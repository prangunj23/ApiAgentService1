...
API_ENV="$CONFIG_DIR/operation.env"
AGENT_ENV="$CONFIG_DIR/service1-agent.env"
ZONE="tailnet"
...
# MagicDNS short name, e.g. apiagent-service1. This is the Host header the UI and the
...
echo
echo "Restarting."
systemctl restart operation.service service1-agent.service 2>/dev/null || {
